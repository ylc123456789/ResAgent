...
- **状态**: ✅ 核心能力验证完成。全链路「search → 设计 → 执行 → 分析」走通、科学结论已产出；暴露 3 个问题（2 已修复、1 最小修复）。
...
| 3 | 重规划不清理旧任务（supersede 缺失，required 旧任务卡 finish） | ResAgent | ✅ 已修复（`3d33cc5`） |
...
## 问题 3（已修复）：重规划不清理旧任务（supersede 缺失）
